...
"xui_mihomo|tell|Подписка mihomo и кнопка XKeen на месте|Роутеры с XKeen забирают все подключения и AmneziaWG одной подпиской mihomo. Путь к ней и кнопку на странице подписки VSM врезает в snippets/includes.conf — файл установщика 3x-ui-pro, который его патч переписывает целиком. Пропажа беззвучна: обычная подписка работает, а роутеры перестают обновляться и получают 404. Возвращает пункт меню X-UI «Роутер (XKeen)»."
"ssh_ladder|fix|Нарастающий бан подбора пароля SSH действует|Вход root по паролю открыт всему интернету: за сутки на главный сервер пришло 2339 неудачных попыток с 14 адресов. Лестница (15 мин, час, сутки, неделя, месяц, год) делает подбор с одного адреса бессмысленным. Включается пунктом «Настройка» → «Бан подбора SSH»; сверка следит только за тем, что включено там, и возвращает молча: пропажа защиты не видна ничем, кроме журнала попыток."
"monthly_updates|fix|Обновления безопасности раз в 30 дней|Образ хостера выключает автообновления сам — файлом в apt.conf.d и масками на юнитах apt, — и уязвимые openssh и openssl остаются на сервере месяцами без единого сообщения. Включается пунктом «Настройка» → «Автообновления»; сверка следит только за тем, что включено там."
)
...
#
# НО СОКЕТ ЕЩЁ И ДОЛЖЕН БЫТЬ ПОРТОМ СОЕДИНЕНИЯ. `ss -l` для UDP показывает
# любой неподключённый сокет, а исходящий UDP xray (freedom) именно такой:
# *:<случайный порт>, и через минуту уже другой. На свежем сервере 25.09.2026
# их было 12 при нуле клиентов — 3x-ui 3.8 сама ходит через свой socks-вход
# xray, — и позиция требовала открыть наружу 12 случайных портов, которые
# живут минуты. Поэтому берём пересечение: сокет слушает (факт) И этот порт
# назначен соединению в базе панели (порт — не протокол, список протоколов
# по-прежнему не наш). База не читается — портов нет, позиция молчит.
_xui_inbound_ports() {
    command -v sqlite3 >/dev/null 2>&1 || return 0
    sqlite3 -cmd ".timeout 5000" "${XUI_DB:-/etc/x-ui/x-ui.db}" \
        "select port from inbounds where port > 0" 2>/dev/null | sort -un
}
_xui_public_udp_ports() {
    local inbound
    inbound="$(_xui_inbound_ports)"
    [ -n "$inbound" ] || return 0
    ss -ulnpH 2>/dev/null \
...
# --- Подписка mihomo --------------------------------------------------
#
# Применима, только когда подписка mihomo включена в панели: выключена —
# кусков в nginx быть и не должно. Состояние читает та же функция, что
# показывает экран меню, — чтобы экран и сверка не разошлись во мнениях.
applies_xui_mihomo() {
    declare -F xui_mihomo_state >/dev/null 2>&1 || return 1
    command -v sqlite3 >/dev/null 2>&1 || return 1
    [ -r "${XUI_DB:-/etc/x-ui/x-ui.db}" ] || return 1
    [ "$(_xm_get subClashEnable)" = "true" ]
}
want_xui_mihomo() { echo "на месте"; }
read_xui_mihomo() {
    local st
    st="$(xui_mihomo_state)"
    if [ "$st" = "on" ]; then
...
# --- Защита сервера: бан подбора SSH и автообновления -----------------
#
# Обе позиции опираются на отметку, которую ставит меню (lib/hardening.sh), как
# journal_limits: без отметки снятая владельцем защита выглядела бы поломкой, а
# исчезнувшая — «вы её и не включали». Класс fix: это безопасность, и вернуть
# своё решение владельца молча безопасно — файлы наши, чужие не правятся.
applies_ssh_ladder() { [ "$(_state_get ssh_ladder)" = "on" ] && declare -F ssh_ladder_state >/dev/null; }
want_ssh_ladder()    { echo "действует"; }
read_ssh_ladder()    { ssh_ladder_state; }
fix_ssh_ladder()     { ssh_ladder_enable; }

applies_monthly_updates() { [ "$(_state_get monthly_updates)" = "on" ] && declare -F updates_state >/dev/null; }
want_monthly_updates()    { echo "раз в ${HARD_UPDATE_DAYS:-30} дн."; }
read_monthly_updates()    { updates_state; }
fix_monthly_updates()     { updates_monthly_enable; }
