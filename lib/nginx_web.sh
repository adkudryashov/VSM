...
web_toml_enable() {
...

    {
        ...
        echo "# carrier: с 04.09.2026 стоит websocket, НА ПРОБЕ до 07.09.2026."
        echo "#"
        echo "# Замер того дня, свежие сессии, одна и та же нагрузка:"
        echo "#   websocket        97.4 МБ/мин, 1 сокет"
        echo "#   websocket-lanes  73.8 МБ/мин, до 13 сокетов под нагрузкой"
        echo "#"
        echo "# Разница в пределах разброса одного прогона, владелец глазами её"
        echo "# не увидел. Решающим доводом стало число сокетов: каждый из них —"
        echo "# отдельное TLS-соединение через nginx, а каждое оставляет около"
        echo "# 100 КБ невозвращаемой памяти (замер 30.08.2026). Тринадцать"
        echo "# против одного на каждый всплеск активности."
        echo "#"
        echo "# Оба переживают zapret2 — это проверено по коду MTProxyL:"
        echo "# web_carrier_survives_zapret2 принимает websocket и websocket-lanes."
        echo "# Не прижилось — вернуть websocket-lanes, правка горячая, без"
        echo "# перезапуска движка."
        echo "[web]"
        echo "enabled = true"
        echo 'carrier = "websocket"'
        echo ""
        echo "[web.debug]"
        echo "enabled = false"
        echo ""
