...
#
# Движок вдобавок требует, чтобы порт был именно 443, и отказывается
# стартовать иначе: web.vhosts[0].public_addr must be a concrete socket
# address on port 443. Поймано пробным движком 08.09.2026 — подменил порт на
# 1443, и он не поднялся. Тогда это записали как новинку 3.5.7; 19.09.2026
# выяснилось, что так же отказывает и 3.5.5.
web_public_addr() {
    local ip
...
web_toml_enable() {
...

    {
        ...
        echo "# carrier: websocket-lanes. Проба websocket 04-07.09.2026 не"
        echo "# прижилась: владелец сказал, что стало хуже. Вернули 08.09.2026."
        echo "#"
        echo "# Замер 04.09, свежие сессии, одна и та же нагрузка:"
        echo "#   websocket        97.4 МБ/мин, 1 сокет"
        echo "#   websocket-lanes  73.8 МБ/мин, до 13 сокетов под нагрузкой"
        echo "#"
        echo "# Цифры были за websocket, ощущения — против. Замер мерил скорость"
        echo "# одной закачки, а не отзывчивость под настоящей работой, и на"
        echo "# этот вопрос он не отвечал. Где замер и человек расходятся, а"
        echo "# замер мерил не то, верим человеку."
        echo "#"
        echo "# Плата за lanes известна: до 13 TLS-соединений через nginx вместо"
        echo "# одного, каждое оставляет около 100 КБ невозвращаемой памяти"
        echo "# (замер 30.08.2026). Это цена, а не дефект."
        echo "#"
        echo "# Оба переживают zapret2 — проверено по коду MTProxyL:"
        echo "# web_carrier_survives_zapret2 принимает websocket и websocket-lanes."
        echo "#"
        echo "# ПРО iOS. Родной клиент без метаданных всегда берёт этот"
        echo "# фиксированный carrier, даже при включённом согласовании, а"
        echo "# Telegram iOS умеет только https (docs/Config_params, 3.5.7)."
        echo "# Появится iOS среди клиентов WEB — ему нужен carrier https либо"
        echo "# список carriers, где https стоит запасным."
        echo "[web]"
        echo "enabled = true"
