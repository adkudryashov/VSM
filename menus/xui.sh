...
function manage_adguard {
    while true; do
        ...
        echo -e "${BLUE}------------------------------------------------------${NC}"
        ui_item        "1" "📥" "Установить/обновить" "AdGuard Home на домене панели, через 443"
...
function manage_xui_profile {
    ...
    while true; do
        ...
        echo ""
        ui_section "ДЕЙСТВИЯ"
        ui_item "1" "📸" "Снять шаблон"    "С этого сервера: подписки, названия, хосты — без секретов"
        ui_item "2" "🧩" "Наложить шаблон" "На свежую панель здесь; копия базы и откат при сбое"
        ui_item "3" "🚚" "Перенести"       "Готовые команды копирования на другой сервер"
        ui_item "X" "🔙" "Назад"
        echo ""
        local ch
