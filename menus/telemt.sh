...
function run_mtproxyl {
    ...
    if [ "$was_installed" -eq 0 ]; then
        # ПАНЕЛИ ЗДЕСЬ НЕ ТРОГАЕМ, и это не упущение.
        #
        # Раньше перед установкой стоял страж исключительности панелей: он
        # спрашивал, удалить ли telemt_panel. Посылка была «выбрал MTProxyL —
        # поставишь и её панель». Установщик MTProxyL свою панель НЕ ставит
        # (проверено 23.08.2026), а с 23.09.2026 VSM её и не поддерживает. То
        # есть страж предлагал снести рабочую админку ради установки
        # инструмента, которому она ничем не мешает. Владелец спросил прямо:
        # «как связан mtproxyl в режиме реаниматора без панели с telemt_panel?»
        # — никак.
        #
        # Управление сюда после exec в установщике автора возвращается: чужой
        # скрипт идёт дочерним bash, и exec заменяет дочерний процесс, а не нас.
        # Проверено контрольным случаем 27.08.2026.
    ...
    if have_cmd mtproxyl; then
        ...
        # Веб-панель эта установка не трогает, поэтому и говорить о ней здесь
        # незачем. Прежде тут стояло предложение поставить MTProxyL-Panel —
        # оно имело смысл, пока страж выше сносил telemt_panel. Нет больше ни
        # стража, ни поддержки той панели.
    else
...
function run_telemt_menu {
    while true; do
        ...
        ui_item "3" "🖥" "Веб-панель"          "telemt_panel: поставить, сменить путь, снять"
        echo ""
