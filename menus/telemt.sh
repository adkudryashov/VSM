...
function run_install {
    ...
    #
    # Раньше она приезжала молча, четвёртым этапом, и отказаться было негде.
    # Вопрос появился, когда панелей было две и установка одной означала
    # немедленное удаление другой стражем исключительности — с заведением
    # системного пользователя, пятнадцати строк NOPASSWD и секретного префикса
    # в nginx по дороге. Каждый такой круг «поставить и снести» — лишний шанс
    # оставить за собой права root у несуществующей службы; именно так однажды
    # и остался /etc/sudoers.d/telemt-panel.
    #
    # Панель теперь одна, но вопрос оставлен: без неё telemt полностью
    # работоспособен, и заводить системного пользователя с правами root тому,
    # кто веб-интерфейсом не пользуется, незачем.
    #
    # Умолчание — ставить: человек, пришедший по README, ждёт веб-интерфейс.
    echo -e "\n${CYAN}Веб-панель управления telemt.${NC}"
    echo -e "${C_DESC}   Без неё telemt и маскировка работают полностью — панель нужна"
    echo -e "   только для управления через браузер. Поставить или снять её"
    echo -e "   можно потом пунктом «Веб-панель».${NC}"
    local want_panel
...
#
# Здесь всё в одном месте: что стоит сейчас, поставить, сменить секретный
# путь, снять совсем.
#
# С 23.09.2026 панель одна — telemt_panel. MTProxyL-Panel VSM больше не ставит
# и не настраивает, но по-прежнему ВИДИТ: установки, где она стоит,
# существуют, и притвориться, что её нет, значит оставить работающую админку с
# правами root без всякого присмотра. Поставить telemt_panel поверх неё можно —
# страж исключительности предложит её снять, и это и есть путь перехода.
# ======================================================================
...
# сменить его до сих пор было НЕЧЕМ: префикс писался один раз при установке, и
# единственным способом отреагировать на утечку оставалась переустановка
# панели.
#
# Меняем в nginx и проверяем фактом, что новый адрес отвечает.
...
function _panel_do_rotate_prefix {
    ...
    if ! panel_telemt_installed; then
        if panel_mtproxyl_installed; then
            # Путь MTProxyL-Panel VSM больше не выдаёт и не меняет: ключ
            # base_path в её конфиге недокументирован, а поддерживать вслепую
            # то, от чего отказались, значит обещать больше, чем делаем.
            echo -e "${YELLOW}Стоит MTProxyL-Panel — VSM её больше не настраивает.${NC}"
            echo -e "${BLUE}   Перейти на telemt_panel: пункт «Веб-панель».${NC}"
        else
    ...
    while true; do
        ...
        if panel_telemt_installed || panel_mtproxyl_installed; then
            ...
            if panel_telemt_installed; then
                ...
                # MTProxyL-Panel VSM больше не ведёт: ни адреса, ни пути он для
                # неё не знает. Молчать нельзя — она работает и слушает, — но и
                # обещать управление ею мы больше не вправе.
                ui_kv "⚠  Поддержка" "VSM её больше не настраивает" 18
                ui_kv "➡  Что делать" "поставить telemt_panel — пункт 1, она заменит" 18
            fi
        ...
        ui_item "X" "🔙" "Назад"
        echo ""
        echo -e "${C_DESC}   Панель на сервере одна: две админки — вдвое больше того, что"
        echo -e "   можно взломать и надо обновлять, ради одной задачи.${NC}"
        echo ""

        read -p "Ваш выбор [1-3, X]: " choice || return
        case "$choice" in
            1) _panel_do_install_telemt; read -p "Нажмите Enter..." ;;
            2) _panel_do_rotate_prefix;  read -p "Нажмите Enter..." ;;
...
function run_diagnostics {
    ...
    # Диагностика обязана называть панель, которая СТОИТ, а не ту, которой нет.
    # Порт печатаем только у своей: у MTProxyL-Panel он другой, а спрашивать её
    # конфиг VSM больше не умеет — и врать номером хуже, чем промолчать.
    if panel_telemt_installed; then
        echo -e "$(printf '%-15s' "$(panel_name_line):") [$(panel_any_status_line)]  порт ${PANEL_PORT}"
    else
        echo -e "$(printf '%-15s' "$(panel_name_line):") [$(panel_any_status_line)]"
    fi
    ...
    if [ -x "$VSM_ROOT/checks/drift.sh" ]; then
        ...
        # Показать мало: сверка прямо говорит «чинится вашим решением», а
        # принять это решение раньше было негде, кроме команды в терминале.
        drift_offer_accept
    fi
...
# ----------------------------------------------------------------------
# ПРИНЯТЬ РАСХОЖДЕНИЕ КАК НОРМУ
#
# Сверка говорит «это чинится вашим решением», но принять решение было негде:
# единственным способом оставалась команда в терминале, о которой владелец
# знать не обязан. Сообщение в Telegram звало в диагностику, а диагностика
# повторяла тот же текст — и круг замыкался. Сказано владельцем 20.09.2026.
#
# Что можно принимать, спрашиваем у самой сверки (поле accept в её машинном
# выводе), а не повторяем список здесь: повторённый однажды разойдётся с
# настоящим, и меню начнёт предлагать принять то, что не принимается.
#
# Позиции без запомненного снимка сюда не попадают вовсе: у них принимать
# нечего, их чинят пунктом меню — «Восстановить nginx» и подобными.
# ----------------------------------------------------------------------
function drift_offer_accept {
    local script="$VSM_ROOT/checks/drift.sh" json count i id title choice
    [ -x "$script" ] || return 0
    if ! command -v jq >/dev/null 2>&1; then
        return 0
    fi
    json=$(bash "$script" --json --dry-run 2>/dev/null) || return 0
    count=$(jq -r '[.items[] | select(.accept == true)] | length' <<< "$json" 2>/dev/null)
    [[ "$count" =~ ^[0-9]+$ ]] || return 0
    [ "$count" -eq 0 ] && return 0

    echo ""
    echo -e "${CYAN}--- ЭТО МОЖНО ПРИНЯТЬ КАК НОРМУ --------------------------${NC}"
    echo -e "Если изменение законное — вы сами что-то поставили или удалили, —"
    echo -e "примите его, и сверка перестанет о нём напоминать. Наблюдение при"
    echo -e "этом остаётся: она просто запомнит новое состояние как исходное."
    echo ""
    for (( i=0; i<count; i++ )); do
        title=$(jq -r --argjson n "$i" '[.items[] | select(.accept == true)][$n].title' <<< "$json")
        echo -e "  ${GREEN}$((i+1))${NC}) $title"
    done
    echo ""
    read -p "Номер позиции (Enter — ничего не менять): " choice
    [[ "$choice" =~ ^[0-9]+$ ]] || return 0
    [ "$choice" -ge 1 ] && [ "$choice" -le "$count" ] || return 0

    id=$(jq -r --argjson n "$((choice-1))" '[.items[] | select(.accept == true)][$n].id' <<< "$json")
    [ -n "$id" ] && [ "$id" != "null" ] || return 0
    echo ""
    bash "$script" --accept "$id"
    read -p "Нажмите Enter для возврата..."
}
...
function show_credentials {
    ...
    if [ ! -f "$STACK_CREDS" ]; then
        ...
        # Стоит MTProxyL-Panel — говорим, где её данные, но не читаем их сами.
        #
        # Прежде мы лезли в её конфиг и печатали адрес с логином. Теперь VSM её
        # не ведёт, и подставлять значения из чужого файла, формат которого
        # больше никто не сверяет, значит однажды напечатать неправду с очень
        # уверенным видом. Пароля там и раньше не было: панель хранит хэш.
        #
        ...
        if panel_mtproxyl_installed; then
            ...
            printf '   %b%s%b%s\n' "$C_NAME" "$(ui_pad 'MTProxyL-Panel:' 20)" "$NC" \
                "VSM её не настраивает — адрес в /etc/mtproxyl-panel/config.toml"
...
function run_mtproxyl {
    ...
    if [ "$was_installed" -eq 0 ]; then
        # Страж ДО запуска чужого установщика.
        #
        # ЗДЕСЬ БЫЛО НАПИСАНО, ЧТО УПРАВЛЕНИЕ К НАМ НЕ ВЕРНЁТСЯ, — И ЭТО НЕВЕРНО.
        # Обоснованием служило то, что установщик автора заканчивается exec-ом
        # своего меню. Но exec заменяет собой ТОТ процесс, в котором выполнен, а
        # чужой скрипт мы запускаем дочерним bash (run_remote_script). Значит
        # заменяется дочерний процесс, и когда человек выходит из меню автора,
        # управление возвращается сюда. Проверено контрольным случаем
        # 27.08.2026: родительский скрипт продолжил работу после exec в дочернем.
        #
        # Спрашивать до всё равно правильно — но по другой причине: удаление
        # чужой панели должно быть решено до того, как что-то начнёт меняться, а
        # не после десяти минут установки. Отказ прерывает установку целиком.
        #
    ...
    if have_cmd mtproxyl; then
        ...
        if ! panel_telemt_installed && ! panel_mtproxyl_installed; then
            # Предлагаем поставить прямо здесь, а не отправляем по командам.
            #
            # Раньше тут печаталась инструкция из трёх строк: выполните
            # mtproxyl panel install, ответьте вариант 2, потом вернитесь в
            # такой-то пункт меню. Три ручных шага после установки, каждый —
            # место, где человек, ставящий по README, останавливается. И
            # молчаливо: до последнего шага панель снаружи не существует вовсе,
            # а почему — нигде не сказано.
            #
            # Всё, что не требует решения человека, должно случаться само;
            # решение здесь одно — нужна ли панель.
            echo -e "\n${YELLOW}❗  Веб-панели на сервере сейчас нет ни одной.${NC}"
            echo -e "${C_DESC:-}   Установщик MTProxyL свою панель не ставит — она отдельной командой."
            echo -e "   VSM может поставить её сейчас и сразу довести до конца:"
            echo -e "   доступ только с петли, секретный путь на 443, проверка ответом.${NC}"
            local want_panel
            read -r -p "$(echo -e "${YELLOW}Поставить веб-панель сейчас? [Y/n]: ${NC}")" want_panel
            case "$want_panel" in
                [Nn]*) echo -e "${C_DESC:-}   Хорошо. Позже — пункт «Веб-панель» этого меню.${NC}" ;;
...
function run_telemt_menu {
    while true; do
        ...
        # Стоит MTProxyL-Panel — говорим об этом один раз и по делу. Адреса у
        # неё VSM больше не знает (ключ base_path читался её собственной
        # библиотекой, которая ушла), а выдумывать нечего: панель работает,
        # просто ведёт её теперь не VSM.
        if panel_mtproxyl_installed; then
            ui_kv '🖥  Веб-панель' 'MTProxyL-Panel — VSM её не настраивает' 20
        fi
        # Ни одной панели — состояние законное, но оставлять человека гадать
        # не надо.
        if ! panel_telemt_installed && ! panel_mtproxyl_installed; then
