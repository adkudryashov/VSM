...
function _panel_do_install_mtproxyl {
    if ! command -v mtproxyl >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}   Сначала пункт «MTProxyL» этого меню, режим Reanimator.${NC}"
        # Предупреждаем заранее: его установщик заканчивается запуском своего
        # меню и сюда не возвращается. Человек, не знающий этого, решает, что
        # что-то сломалось, и начинает искать несуществующую причину.
        echo -e "${C_DESC:-}   Его установщик откроет собственное меню и сюда не вернётся —"
        echo -e "   выйдите из него и зайдите в «Веб-панель» снова.${NC}"
        return 1
    fi
    ...
    # ДОВОДИМ ДО КОНЦА ЗДЕСЬ, А НЕ ОТПРАВЛЯЕМ ЧЕЛОВЕКА ПО ПУНКТАМ МЕНЮ.
    #
    # Раньше после чужого установщика оставались два обязательных шага, и оба
    # человек делал руками: сходить в «Восстановить nginx» за секретным путём и
    # в «Учётные данные» за адресом. Панель до первого из них снаружи не
    # существует вовсе. Владелец на это и указал: с провожатым пройти можно, а
    # тот, кто ставит по README, застрянет — установщик отрапортовал об успехе,
    # а панели по адресу нет, и почему — нигде не сказано.
    #
    # Ни одно из этих действий не требует решения человека. Всё, что не требует
    # решения, обязано случиться само.
    echo -e "\n${CYAN}>>> Привожу панель к решениям VSM...${NC}"
    # Умолчания чужого установщика правит сверка, а не мы: она делает резервные
    # копии, объясняет каждую правку и одна на все пути установки. Заодно
    # чинится и неверный ответ на его вопрос про доступ — панель вернётся на
    # loopback, даже если выбрали «со всех интерфейсов». Это важно сделать ДО
    # выдачи пути: панель, висящую на всех интерфейсах, прятать за префикс
    # бессмысленно, и mtpl_restore_proxy справедливо откажется.
    #
    # ПЕРВЫЙ ПРОГОН МОЛЧА, и вот почему. Секретного пути на этот момент ещё нет,
    # поэтому сверка честно сообщает о двух расхождениях — нет блока в nginx и
    # нет префикса. Оба чинит следующая же строка. Показывать человеку «требует
    # вашего решения: 2» про то, что исправится через секунду, значит пугать
    # зря и приучать не верить сверке. Итог он увидит ниже, и настоящий.
    local drift="${VSM_ROOT}/checks/drift.sh"
    [ -f "$drift" ] && bash "$drift" >/dev/null 2>&1
    # Секретный путь: создаёт при первой настройке, восстанавливает потом.
    # Та же функция, что зовёт «Восстановить nginx», — второй копии нет.
    mtpl_restore_proxy
    # А вот теперь — настоящий итог, и уже с путём на месте.
    if [ -f "$drift" ]; then
        echo -e "\n${CYAN}>>> Итог сверки с реестром:${NC}"
        bash "$drift" 2>&1 | tail -n 4
    fi
    echo -e "\n${GREEN}✓ Панель готова. Адрес и логин — пункт «Учётные данные».${NC}"
    return 0
}
