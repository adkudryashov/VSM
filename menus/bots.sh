...
function manage_beszel {
    ...
    echo ""
    echo -e "${YELLOW}Нужна учётная запись В САМОМ beszel${NC} — введите её здесь."
    echo -e "Пароль не печатается и в список процессов не попадает."
    echo ""
    echo -e "${YELLOW}Важно про отдельную запись для бота:${NC} в beszel сервер видит"
    echo -e "тот, кто его добавил. Свежесозданному пользователю не видно НИ ОДНОГО"
    echo -e "сервера, пока вы не отдадите их ему в интерфейсе хаба. Проще и надёжнее"
    echo -e "указать здесь ту запись, под которой серверы заводились."
    echo ""
