...
тексте шаблона разом. Так «🇩🇪 alice MyVPS» становится «{FLAG} alice {NAME}»,
...
# --------------------------------------------------------------- названия
#
# Названия — правило VSM, а не вкус шаблона (решение владельца 25.09.2026;
# накануне было «флаг тип», без сервиса):
#
#   входящий  «флаг сервис тип»      🇩🇪 MyVPS xhttp
#   подписка  «флаг сервис клиент»   🇩🇪 MyVPS alice
#
# Сервис во входящем нужен и роутеру: имена прокси в подписке mihomo берутся
# из названий входящих, и у двух серверов одной страны «🇩🇪 awg» совпали бы.
#
...
# подписки жило в шаблоне как есть — «{FLAG} alice {NAME}» с именем клиента,
# вбитым буквально, — и у любого другого клиента подписка называлась бы alice.
...
def apply_naming(profile):
    ...
    for ib in profile["inbounds"]:
        ...
        ib["remark"] = " ".join(x for x in ("{FLAG}", "{NAME}", kind) if x)
    profile["settings"]["subTitle"] = SUB_TITLE
    return profile


def name_from_title(title):
    """Сервис из названия подписки «🇩🇪 MyVPS {{EMAIL}}» → «MyVPS». Запасной
    путь: сервер, где входящие названы без сервиса (правило 24.09.2026)."""
...
def guess_identity(inbounds):
    """Флаг и имя сервера из названия входящего reality: «🇩🇪 MyVPS reality»."""
    for ib in inbounds:
