...
def test_экран_таблицей_со_столбцами_как_в_сводке():
    ...
    assert "FST.KZ" in богатый and "VEESP" in богатый
...
def test_запасной_текст_собран_отдельно_а_не_срезан():
    ...
    """
    v = разбор.read_verdict(ответ(сервер("FST.KZ")), СЕЙЧАС)
...
def test_строка_про_упавшие_службы_только_когда_они_есть():
    ...
    """
    чисто = разбор.read_verdict(ответ(сервер("FST.KZ", sv=(59, 0))), СЕЙЧАС)
    богатый, запасной = разбор.parts(чисто)
    assert "Упавшие службы" not in богатый
    assert "Упавшие службы" not in запасной

    беда = разбор.read_verdict(ответ(сервер("FST.KZ", sv=(59, 2))), СЕЙЧАС)
...
def test_экран_помечает_молчащего():
    v = разбор.read_verdict(ответ(сервер("P2GO", возраст=600)), СЕЙЧАС)
    богатый, запасной = разбор.parts(v)
