...
def test_всё_в_порядке():
    v = разбор.read_verdict(ответ(сервер("alpha"), сервер("beta")), СЕЙЧАС)
    assert v.hub_ok
    assert v.silent == [] and v.offline == []
    assert [r.name for r in v.rows] == ["alpha", "beta"]
    assert all(r.alive for r in v.rows)


def test_молчащий_сервер_замечен():
    v = разбор.read_verdict(ответ(сервер("gamma", возраст=400)), СЕЙЧАС)
    assert v.silent == ["gamma"]
...
def test_свежий_доклад_не_считается_молчанием():
    ...
    v = разбор.read_verdict(ответ(сервер("gamma", возраст=299)), СЕЙЧАС)
    assert v.silent == []


def test_упавший_по_мнению_хаба():
    v = разбор.read_verdict(ответ(сервер("delta", status="down", возраст=900)), СЕЙЧАС)
    assert v.offline == ["delta"]
...
def test_приостановленный_не_авария():
    ...
    """
    v = разбор.read_verdict(ответ(сервер("gamma", status="paused", возраст=99999)), СЕЙЧАС)
    assert v.silent == [] and v.offline == []
    assert v.trouble == []
...
def test_info_строкой_тоже_разбирается():
    ...
    v = разбор.read_verdict(ответ(сервер("alpha", info_строкой=True, cpu=7.5)), СЕЙЧАС)
...
def test_экран_таблицей_со_столбцами_как_в_сводке():
    """
    Образцом взята сводка 3x-ui: те же столбцы таблицей, а не построчный
    текст. Построчный рядом со сводкой выглядел чужеродно — сказано владельцем
    по снимку экрана 20.09.2026.
    """
    v = разбор.read_verdict(ответ(сервер("alpha"), сервер("beta")), СЕЙЧАС)
    богатый, запасной = разбор.parts(v)
    assert "<table>" in богатый
    for столбец in ("Сервер", "CPU", "RAM", "DISK", "UPTIME"):
        assert f"<th>{столбец}</th>" in богатый
    assert "alpha" in богатый and "beta" in богатый
    assert "CPU" in запасной and "RAM" in запасной and "DISK" in запасной


def test_запасной_текст_собран_отдельно_а_не_срезан():
    """
    Срезать теги регуляркой нельзя: таблица превращается в слипшуюся строку
    значений без разделителей, и случается это ровно тогда, когда что-то уже
    сломалось. Признак сборки отдельным проходом — разделители между полями.
    """
    v = разбор.read_verdict(ответ(сервер("alpha")), СЕЙЧАС)
    _, запасной = разбор.parts(v)
    assert "<table>" not in запасной
    assert "·" in запасной


def test_строка_про_упавшие_службы_только_когда_они_есть():
    """
    Раздел, который всегда говорит одно и то же, читать перестают. «Упавших
    нет» у каждого сервера занимало четверть экрана и не сообщало ничего.
    """
    чисто = разбор.read_verdict(ответ(сервер("alpha", sv=(59, 0))), СЕЙЧАС)
    богатый, запасной = разбор.parts(чисто)
    assert "Упавшие службы" not in богатый
    assert "Упавшие службы" not in запасной

    беда = разбор.read_verdict(ответ(сервер("alpha", sv=(59, 2))), СЕЙЧАС)
    богатый, запасной = разбор.parts(беда)
    assert "Упавшие службы (1)" in богатый
    assert "2 из 59" in богатый


def test_экран_помечает_молчащего():
    v = разбор.read_verdict(ответ(сервер("gamma", возраст=600)), СЕЙЧАС)
    богатый, запасной = разбор.parts(v)
    assert "молчит" in богатый and "молчит" in запасной
    # У молчащего сервера цифр нет вовсе: показать последние известные значит
    # выдать старое за нынешнее.
    assert "<td>—</td>" in богатый


def test_экран_про_отказ_в_пароле_не_говорит_про_аварию():
    """
    Разные беды — разные слова. Иначе владелец пойдёт поднимать сервер,
    который прекрасно работает.
    """
    богатый, _ = разбор.parts(разбор.read_verdict(ответ(state="rejected"), СЕЙЧАС))
    assert "не принимает учётную запись" in богатый
    assert "не отвечает" not in богатый


def test_экран_пустого_хаба_объясняет_себя():
    """
    Пустой список значит либо «серверов нет», либо «они у другой учётной
    записи» — снаружи это неразличимо. Названы обе причины: в beszel сервер
    видит тот, кто его добавил, и заведённой отдельно записи бота не видно
    ничего. Проверено 20.09.2026 на живом хабе: все четыре сервера числились
    за первым пользователем.
    """
    богатый, запасной = разбор.parts(разбор.read_verdict(ответ(), СЕЙЧАС))
    for текст in (богатый, запасной):
        assert "не заведены" in текст
        assert "другой учётной записи" in текст


def test_имя_сервера_экранируется():
    """Имя серверу даёт человек в вебе, и оно попадает в HTML сообщения."""
    v = разбор.read_verdict(ответ(сервер("<b>ой</b>")), СЕЙЧАС)
    богатый, запасной = разбор.parts(v)
    assert "&lt;b&gt;" in богатый
    assert "&lt;b&gt;" in запасной
