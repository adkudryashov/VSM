...
def test_экран_показывает_все_серверы():
    v = разбор.read_verdict(ответ(сервер("FST.KZ"), сервер("VEESP")), СЕЙЧАС)
    текст = разбор.render(v)
    assert "FST.KZ" in текст and "VEESP" in текст
    assert "упавших нет" in текст


def test_экран_помечает_молчащего():
    v = разбор.read_verdict(ответ(сервер("P2GO", возраст=600)), СЕЙЧАС)
    assert "молчит" in разбор.render(v)
...
def test_экран_про_отказ_в_пароле_не_говорит_про_аварию():
    ...
    """
    текст = разбор.render(разбор.read_verdict(ответ(state="rejected"), СЕЙЧАС))
    assert "не принимает учётную запись" in текст
    assert "не отвечает" not in текст
...
def test_экран_пустого_хаба_объясняет_себя():
    ...
    """
    текст = разбор.render(разбор.read_verdict(ответ(), СЕЙЧАС))
