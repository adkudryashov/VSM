...
def test_названия_стали_своими_на_новом_сервере(наложенный):
    ...
    # Правило VSM: входящий — «флаг сервис тип», подписка — «флаг сервис клиент».
...
def test_сервис_переживает_перенос_по_новому_правилу(tmp_path, заглушки, наложенный):
    """
    Сервис с одного сервера не должен доехать до следующего текстом: снятие
    обязано превратить его в метку, наложение — подставить новый. Иначе на
    следующем сервере подписка и входящие назовутся чужим сервисом.
    """
    ...
    assert ответ.returncode == 0, ответ.stdout + ответ.stderr
    assert "🇫🇮 Aeza xhttp" in входящие(db)
