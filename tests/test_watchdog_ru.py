...
# --- РАСПИСАНИЕ ПРОВЕРКИ ВНЕШНЕГО АДРЕСА -----------------------------
#
# Прежде адрес спрашивался каждый такт — 1440 обращений в сутки к чужому
# сервису с сервера, который маскируется, и 7 МБ утечки в час: каждый
# внешний вызов по HTTPS оставляет около 0.12 МБ невозвращаемой памяти.
# Тесты держат расписание, потому что убрать его — правка в одну строку,
# а последствия видны только замером RSS через сутки.

def test_адрес_спрашивается_один_раз_за_интервал(сторож, monkeypatch):
    monkeypatch.setattr(settings, "IP_CHECK_INTERVAL_MINUTES", 30)
    bot = ЗаглушкаБота()
    спрошено = []

    async def подделка(*_a, **_k):
        спрошено.append(1)
        return "203.0.113.10"

    monkeypatch.setattr(monitor.globalping, "public_ip", подделка)

    async def сценарий():
        for _ in range(5):          # пять тактов сторожа подряд
            await сторож._poll_ip(bot)

    прогнать(сценарий())
    assert len(спрошено) == 1, "за один интервал спрашиваем ровно один раз"


def test_после_интервала_спрашиваем_снова(сторож, monkeypatch):
    """
    Обратная сторона: без неё проверка выше прошла бы и у кода, который
    не спрашивает адрес никогда.
    """
    monkeypatch.setattr(settings, "IP_CHECK_INTERVAL_MINUTES", 30)
    bot = ЗаглушкаБота()
    спрошено = []
    ...
    async def сценарий():
        await сторож._poll_ip(bot)
        сторож._ip_next = time.time() - 1      # срок вышел
        await сторож._poll_ip(bot)
...
def test_первый_такт_после_старта_спрашивает_сразу(сторож, monkeypatch):
    """
    Ноль в _ip_next означает «спросить немедленно»: не зная адреса, сверка
    домена молчит, и ждать полчаса после запуска нечего.
    """
    monkeypatch.setattr(settings, "IP_CHECK_INTERVAL_MINUTES", 30)
    bot = ЗаглушкаБота()
    спрошено = []
    ...
    monkeypatch.setattr(monitor.globalping, "public_ip", подделка)
    assert сторож._ip_next == 0.0, "предпосылка проверки"
    прогнать(сторож._poll_ip(bot))
    assert len(спрошено) == 1
