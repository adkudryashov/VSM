...
for _lib in nginx_mask.sh nginx_panel_proxy.sh panels.sh cert_renew.sh xui_mihomo.sh hardening.sh expectations.sh; do
...
if [ "$DRIFT" -gt 0 ]; then
    ...
    echo "  Блоки nginx возвращаются пунктом «Восстановить nginx» в меню telemt."
