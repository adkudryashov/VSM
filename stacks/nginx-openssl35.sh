...
# Префикс своего OpenSSL. Его же ищет _pq_openssl_bin в menus/telemt.sh, когда
...
# stacks/telemt.sh: скрипт самодостаточен и общий слой меню не подключает.
...
#
# Почему не LD_LIBRARY_PATH: _pq_openssl_bin в menus/telemt.sh вызывает
...
# menus/telemt.sh искал ${OPENSSL_PREFIX}/bin/openssl — и PQ-часть пункта
...
# menus/telemt.sh. Именно эта проверка и поймала отсутствие rpath: make
...
# оставить сервер без прокси. Тот же приём, что вокруг certbot в menus/setup.sh.
