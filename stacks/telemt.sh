...
#
# Если панель не встанет — XUI_VERSION=3.7.0 возвращает последнюю версию, на
# которой стек проходил приёмку целиком (стенд живёт на ней с 27.08.2026).
# Это флаг на такой случай, а не новое умолчание. Для версий до 3.8.0
# установщик 3x-ui-pro берётся с прежнего коммита — почему, см. этап 1.
#
# С 16.09.2026 без флага приезжает 3.8.x, и на ней стек целиком НЕ проверялся.
# Что в ней задевает нас, разобрано в docs/DECISIONS.md.
...
# Автопродление сертификата. Не обязательна по той же причине, что и страж
# панелей: на не до конца обновлённой установке её может не быть, и ронять
# из-за этого развёртывание стека незачем.
if [[ -f "$VSM_LIB/cert_renew.sh" ]]; then
    # shellcheck disable=SC1091
    . "$VSM_LIB/cert_renew.sh"
fi
...
#
# Те же две строки стоят в lib/nginx_web.sh и в _atomic_replace из
...
if [[ "$MODE" == "full" ]]; then
    ...
    # Старая 3x-ui требует и старого установщика. С 28030b3 (16.09.2026) автор
    # пишет пути подписок через UPDATE, а не INSERT: строки subPath и
    # subJsonPath заводит сама панель, но только с 3.8.0
    # (seedRandomSubscriptionPaths). На 3.7.0 и раньше их нет, UPDATE не
    # находит ни одной строки и молча ничего не меняет — панель раздаёт
    # подписки по умолчательному пути, а nginx ждёт их по случайному. Ошибки
    # не будет нигде, просто перестанут обновляться подписки у клиентов.
    # c8c1287 — последний коммит до этой правки.
    if [[ -n "${XUI_VERSION:-}" ]] \
       && [[ "$(printf '%s\n' "${XUI_VERSION#v}" 3.8.0 | sort -V | head -1)" != "3.8.0" ]]; then
        XUI_URL="https://raw.githubusercontent.com/mozaroc/3x-ui-pro/c8c1287/x-ui-latest.sh"
        log "  3x-ui ${XUI_VERSION} старше 3.8.0 — установщик 3x-ui-pro берём с c8c1287"
    fi
...
# Владелец мог отказаться от панели ещё при вводе параметров. Ставить её тому,
# кто ею не пользуется, значит завести системного пользователя, пятнадцать
# строк NOPASSWD и секретный префикс в nginx — и всё это ради страницы, куда
# никто не зайдёт.
...
if [[ "${INSTALL_WEB:-1}" -eq 1 ]]; then
    ...
    if web_toml_enable "$DOMAIN_PANEL"; then
        if web_engine_restart_verified "$DOMAIN_PANEL" "$TELEMT_PORT"; then
            ...
            # Называем причину. Эта ветка однажды уже соврала: проверка
            # сертификата сравнивала текст subject, формат которого зависит от
            # версии OpenSSL, и на Ubuntu 24.04 отказывала всегда. Сообщение
            # при этом обвиняло движок, который поднимался прекрасно, и искать
            # было негде. Хвост журнала стоит двух строк кода.
            warn "WEB Proxy: движок не поднялся с новым конфигом — откатываю"
            warn "  что сказал движок:"
            journalctl -u telemt -n 15 --no-pager 2>&1 | sed 's/^/      /' >&2
...
# ---------------------------------------------------------------------------
# ЭТАП 7 — автопродление сертификата
# ---------------------------------------------------------------------------
# Сертификат к этому моменту уже выпущен (этап 1) и nginx настроен. Способ
# выпуска — standalone, он же запоминается для продления, а при продлении порт
# 80 занят работающим nginx, и попытка падает молча. Разбор — в шапке
# lib/cert_renew.sh.
#
# НЕ ФАТАЛЬНО: сертификат действует 90 дней, и стек без этой настройки
# работает. Но молчать нельзя — цена в том, что через два месяца разом
# отвалится всё, что стоит за nginx.
if declare -F cert_renew_apply >/dev/null 2>&1; then
    log "Этап 7: автопродление сертификата"
    if cert_renew_apply; then
        log "Продление переведено на webroot, nginx гасить больше не нужно"
    else
        warn "Автопродление настроить не удалось — проверьте: certbot renew --dry-run"
    fi
fi
