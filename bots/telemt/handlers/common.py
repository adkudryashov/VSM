...
# Доля, начиная с которой наши собственные зонды стоят места на экране.
#
# Их сорок в час, всегда. На спокойном сервере это больше половины строки, и
# тогда о них надо сказать: иначе владелец считает посторонними тех, кого сам
# же и создал. На фоне всплеска в тысячи это 3%, и упоминание лишь удлиняет
# строку, которая и так не помещается в одно сообщение.
STRAY_OWN_SHARE = 0.2
...
def _stray_line(total: int, uptime_seconds: float, own_per_hour: float = 0.0) -> str:
    ...
    ПО ОДНОЙ ПОДРОБНОСТИ ЗА РАЗ. Первая редакция говорила всё сразу и занимала
    в Telegram две строки — владелец увидел и сказал «длинная». Длинная строка
    в сводке хуже неполной: её перестают читать целиком, и тогда не работает
    ни одна её часть. Поэтому подробность выбирается по обстановке:

    * скорость выше порога — показываем скорость и то, что это вне оценки.
      Именно здесь молчаливое исключение опасно: сменись однажды секрет, отказы
      всех клиентов лягут сюда же, и сводка покажет «успешных 100%» при мёртвом
      прокси. Крупное число обязано объяснять, почему оно никого не пугает;
    * спокойно, но больше пятой части — наши собственные зонды. Не вычитаем их
      из общего числа: количество СЧИТАНО из настроек, а не измерено, и
      подмешивать расчётное к измеренному в одну цифру значит потерять разницу
      между ними навсегда;
    * спокойно и зондов мало — просто скорость.
    ...
    if high:
        return f"{line} · {format_connections(round(rate))}/час · вне оценки"
    if own_total >= 1 and own_total >= total * STRAY_OWN_SHARE:
        return f"{line} · наши зонды ≈{format_connections(round(own_total))}"
    if hours > 0:
        return f"{line} · {format_connections(round(rate))}/час"
    return line
