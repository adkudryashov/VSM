...
async def cmd_metrics(message: types.Message):
    try:
        ...
        # РАЗЛИЧАЕМ «СЛОМАЛОСЬ» И «НЕ ВКЛЮЧЕНО».
        #
        # Экран показывал голое «Connection refused», и это читается как
        # поломка: человек идёт чинить то, чего не ломали.
        #
        # ЗДЕСЬ ДОЛГО СТОЯЛА НЕПРАВДА. Было написано, что экспортёра в этой
        # сборке telemt нет вовсе — «ни флага в бинаре, ни ключа в конфиге,
        # ни /metrics на порту API». Верным был только третий пункт, и вывод
        # из него сделан неверный: экспортёр слушает СВОЙ порт, а проверка
        # стучалась в порт API. Замерено 08.09.2026 пробными движками:
        # 3.5.5 отдаёт 287 рядов, 3.5.7 — 616, из них 328 про WEB.
        #
        # Включается ключами [server] metrics_listen и metrics_whitelist,
        # по умолчанию выключен. Горячим перечитыванием не подхватывается —
        # нужен перезапуск движка.
        if isinstance(e, (httpx.ConnectError, httpx.ConnectTimeout, OSError)):
            await send_long_message(
                message,
                "📉 <b>Экспортёр метрик не отвечает</b>\n\n"
                f"Адрес: <code>{html.escape(PROMETHEUS_URL)}</code>\n\n"
                "<i>Экспортёр в движке есть, но по умолчанию выключен и слушает "
                "отдельный порт, а не порт API. Выключен — это не поломка.</i>\n\n"
                "<b>Как включить.</b> В <code>/etc/telemt/telemt.toml</code>, "
                "секция <code>[server]</code>:\n"
                '<code>metrics_listen = "127.0.0.1:9090"</code>\n'
                '<code>metrics_whitelist = ["127.0.0.1/32"]</code>\n\n'
                "Затем перезапустить движок: горячим перечитыванием эти ключи "
                "не подхватываются. Порт должен совпадать с "
                "PROMETHEUS_METRICS_URL в bots/.env.\n\n"
