...
class Watchdog:
    def __init__(self):
        ...
        # Момент запуска сторожа. Нужен как ЗАПАСНОЙ якорь свежести
        # чужого вердикта: сразу после перезапуска ru_checked_at ещё
        # ноль, и возраст, посчитанный от него, дал бы полвека — то
        # есть тревогу о молчании проверки на каждом рестарте бота.
        # Считаем от старта: не знаем — значит даём время узнать.
        self._started_at: float = time.time()
        # Когда можно следующую проверку из РФ. Берётся из реестра трат, а не с
        # нуля: иначе КАЖДЫЙ перезапуск бота запускал бы проверку немедленно,
        # не глядя на расписание. За один сегодняшний день обновлений это дало
        # пять лишних прогонов — то есть лишние российские зонды к серверу,
        # который маскируется. Реестр трат переживает перезапуск, поэтому
        # расписание восстанавливается точно.
        #
        # Ноль, если не тратили ни разу: на свежей установке первую проверку
        # надо сделать сразу, иначе полчаса непонятно, работает ли она вообще.
        last = self.state.quota.last_spend()
        self._ru_next: float = (
        ...
        # Когда следующий раз спрашивать свой внешний адрес. Ноль —
        # сразу при старте: не зная адреса, сверка домена молчит.
        self._ip_next: float = 0.0
        # Пришла тревога — карточку надо перенести вниз, под неё. Флаг, а не
        # перенос на месте: за один проход тревог бывает несколько, и каждая
        # удаляла бы и пересоздавала карточку заново.
        self._card_move: bool = False
    ...
    async def _notify(self, bot: Bot, text: str, force: bool = False) -> None:
        ...
        for admin_id in settings.ADMIN_IDS:
            try:
                ...
                self._card_move = True

    async def _move_card_down(self, bot: Bot) -> None:
        """
        Переносит карточку сторожа вниз, под пришедшую тревогу.

        ЗАЧЕМ. Карточка — не сообщение, а место: человек возвращается за
        кнопками именно к ней. Тревога приходит отдельным сообщением (и это
        правило: ночное падение обязано быть видно в истории, а не спрятано
        внутри правки), поэтому карточка после каждой тревоги оказывается
        выше — и за «Обновить» или «Проверить» приходится листать вверх.

        Правка на месте тут не поможет: сообщение остаётся там, где было.
        Только удалить и отправить заново — это и делает relocate.

        ИМПОРТ ВНУТРИ ФУНКЦИИ НЕ ЛЕНЬ. card импортирует monitor ради
        состояния, и обычный импорт наверху замкнул бы кольцо.
        """
        if not self._card_move:
            return
        self._card_move = False
        try:
            from telemt.watchdog import card
            await card.refresh(bot, relocate=True)
        except Exception as exc:
            # Перенос — удобство, а не тревога. Ошибка здесь не должна ронять
    ...
    async def poll_once(self, bot: Bot) -> None:
        ...
        async with self._lock:
            ...
            _save_state(self.state)
            # Тревоги ушли — карточка уехала вверх. Возвращаем её вниз,
            # под них, чтобы кнопки остались под рукой.
            await self._move_card_down(bot)
    ...
    async def _poll_ip(self, bot: Bot) -> None:
        """
        Свой внешний адрес — по расписанию, а не каждый такт.

        Прежде спрашивался каждые 60 секунд. Это давало 1440 обращений в
        сутки к стороннему сервису с сервера, который маскируется, и было
        главным остатком утечки памяти: внешний вызов по HTTPS стоит около
        0.12 МБ невозвращаемых, то есть 7 МБ в час. Адрес при этом меняется
        при переезде сервера — событие, которого может не случиться вовсе.

        Известный адрес живёт в состоянии и переживает перезапуск, поэтому
        сверка домена между опросами работает по нему, как и раньше.
        """
        now = time.time()
        if now < self._ip_next:
            return
        self._ip_next = now + max(int(settings.IP_CHECK_INTERVAL_MINUTES), 1) * 60
    ...
    async def _poll_mtproxyl_verdict(self, bot: Bot) -> None:
        ...
        ЛЮБОЙ ИСХОД ЗАКАНЧИВАЕТСЯ ПРОВЕРКОЙ СВЕЖЕСТИ. Способов остаться без
        измерения три — файл не читается, в нём чужая ошибка, метка не
        двигается, — и раньше каждый вёл к своему `return`, а сказать «мы
        ослепли» было некому. Условие у всех трёх одно и то же: сколько прошло
        с последнего вердикта, которому мы поверили. Поэтому оно и считается в
        одном месте, после разбора, а не в каждой ветке по-своему.
        """
        verdict = mtproxyl.read_verdict()
        if verdict is None:
            # Файл пропал или не разобрался. Прежнее значение ru_error не
            # трогаем: перетереть причину, названную MTProxyL, нашим общим
            # «нет данных» значит потерять единственную подсказку.
            pass
            ...
            if checked_at and checked_at <= self.ru_checked_at:
                # Тот же вердикт, что в прошлый раз. Молчим: повторно проводить
                # его через машину инцидентов значило бы копить плохие опросы на
                # одном и том же измерении и поднять тревогу на пустом месте.
                pass
            else:
                self.ru_error = ""
                await self._apply_ru_verdict(bot, verdict)

        await self._check_ru_stale(bot)

    async def _check_ru_stale(self, bot: Bot, *, watching: bool = True) -> None:
        """
        Чужой вердикт перестал обновляться — значит, встал таймер MTProxyL.

        ЗАЧЕМ ЭТО ТРЕВОГА, А НЕ СТРОКА В КАРТОЧКЕ. Сторож сам доступность не
        меряет — он читает вердикт MTProxyL. Встал чужой таймер, и сторож
        замолкает про вход целиком: тревоги нет, потому что нет и измерения.
        Молчание становится неотличимо от «всё хорошо» ровно в том месте, ради
        которого сторож и заведён. До 28.08.2026 об этом говорилось только в
        карточке — то есть лишь тому, кто сам решил посмотреть. Это тот же
        дефект, ради которого делали heartbeat для самого бота, и лечится он
        так же: молчание обязано звонить.

        Своим измерением молча не подменяем: это вернуло бы двойной расход
        российских зондов у сервера, который маскируется, а о таком владельца
        спрашивают.

        ВОЗРАСТ СЧИТАЕТСЯ ОТ ПОЗДНЕЙШЕГО из двух: последнего принятого вердикта
        и запуска сторожа. Второе — на случай, когда вердикта не было вовсе:
        после перезапуска бота ru_checked_at ещё ноль, и без якоря тревога
        уходила бы на каждом рестарте. Цена — после перезапуска мы даём чужому
        таймеру полный срок заново, то есть уже висящее молчание заметим на
        RU_CHECK_STALE_MINUTES позже. Размен в пользу тишины: ложная тревога
        при каждом обновлении бота обесценила бы все остальные.

        watching=False означает «чужой вердикт нам больше не нужен» — проверку
        выключили или сторож меряет сам. Тогда висевшая тревога снимается:
        оставить её значило бы требовать чинить то, чего мы не спрашиваем.
        """
        limit = max(int(settings.RU_CHECK_STALE_MINUTES), 1) * 60
        age = time.time() - max(self.ru_checked_at, self._started_at)
        stale = watching and age > limit
        if stale:
            self.ru_error = (f"проверка MTProxyL не обновлялась {int(age / 60)} мин — "
                             f"включить: mtproxyl availability on")

        await self._fire_or_clear(
            bot, self.state.ru_stale.update(is_bad=stale), self.state.ru_stale,
            fire="🚨 <b>ДОСТУПНОСТЬ ИЗ РОССИИ НЕ ПРОВЕРЯЕТСЯ</b>\n"
                 f"Последнее измерение {int(age / 60)} мин назад при сроке "
                 f"{int(limit / 60)}.\n"
                 "Сторож не меряет сам — он читает вердикт MTProxyL, а тот "
                 "перестал обновляться. Про доступность из России сторож "
                 "сейчас не знает ничего, и его молчание об этом не говорит.\n"
                 "Включить: <code>mtproxyl availability on</code>",
            clear="✅ <b>ПРОВЕРКА ДОСТУПНОСТИ ВЕРНУЛАСЬ</b>\n"
                  "Вердикт MTProxyL снова обновляется.",
        )
        _save_state(self.state)
    ...
    async def _maybe_check_ru(self, bot: Bot) -> None:
        if not settings.RU_CHECK_ENABLED:
            # Проверку выключили намеренно — тревога о том, что она молчит,
            # стала бы требованием чинить то, чего мы не спрашиваем.
            await self._check_ru_stale(bot, watching=False)
            return
        ...
        # Сторож меряет сам — например, MTProxyL сняли. Чужой вердикт больше не
        # при чём, и висевшая тревога о его молчании снимается: за расписание
        # собственной проверки отвечаем мы, а не он.
        await self._check_ru_stale(bot, watching=False)
        now = time.time()
