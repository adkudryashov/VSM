...
```
отбор модулей   xslt / image_filter / perl / geoip (http и stream) / mail
                вырезаются, если их директив нет в nginx -T; иначе остаются
                + dev-пакеты. stream, stream_ssl и stream_ssl_preread
                обязательны — иначе die до изменений в системе
этап 1          OpenSSL → make install_sw в /opt/openssl-3.5 (нужен пункту 5
                меню telemt для проверки PQ), два -Wl,-rpath, проверка
                запуском бинарника ГОЛЫМ, без LD_LIBRARY_PATH
этап 2          ./configure с флагами дистрибутива, включая cc-opt/ld-opt
этап 3          бэкап бинарника и modules-enabled (не перезаписываются
                повторно), бэкап nginx.conf (каждый раз); подмена;
                снимаются только конфликтующие модули — сторонние из
                nginx-extras сохраняются; nginx -t и старт с откатом;
                парный подъём telemt и telemt-panel (Requires=nginx.service
                их уносит) с проверкой NRestarts, trap на INT TERM HUP;
                итог печатается по факту
...
```
этап 1   Docker; демон проверяется по факту, а не по установленному пакету
этап 2   порт: свой или подобранный; 443/udp запрещён (место под HTTP/3)
...
этап 5   образ сервера по digest
этап 6   ключ сервера + awg-tool gen --version 2.0|3.0. Версия проверяется
         ...
         Плюс фильтр совпадения длин служебных пакетов: S2≠S1+56, S3≠S2+28,
         S3≠S1+84 — иначе два типа пакета одинаковой длины в проводе
этап 7   снятие прежнего резолвера: контейнер убран, Docker 29 запрещает
         доступ к адресам контейнеров в raw PREROUTING, обойти нечем
этап 8   сервер в host-режиме — при публикации порта Docker обходит UFW
этап 9   правило UFW
этап 10  приёмка: запущен, порт слушает, config-applied в журнале;
         затем дедуп правил фаервола, которые контейнер за собой не снимает
--mode uninstall   снимает всё, включая правила фаервола, и проверяет по факту
--mode dedupe      только дедуп правил; зовётся из меню при входе
```
