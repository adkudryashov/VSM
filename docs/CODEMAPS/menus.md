...
`/etc/vsm/telemt.conf`, а не спрашиваются. Ручная проверка не чаще раза в
минуту, траты пишутся в реестр скользящим часом по фактическому числу зондов,
а отказ сервиса блокирует проверки на названный им срок.
