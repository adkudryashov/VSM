...
function awg_clients {
    ...
    case "$ch" in
        1|2)
            ...
            # Внешний адрес подставляем, а не требуем набирать.
            #
            # Прежде его спрашивали каждый раз, и обоснование было неверным:
            # «сервер его не знает». Локальный адрес интерфейса действительно
            # не годится — конфиг с ним не работает нигде, кроме самого
            # сервера, — но ВНЕШНИЙ адрес известен, его же показывает главное
            # меню, и берётся он оттуда же.
            #
            # Вопрос всё равно задаётся, потому что домен — не то же самое, что
            # адрес: по домену клиента можно переселить на другой сервер, не
            # перевыпуская конфиги, и домен переживает смену IP. Поэтому
            # предлагаем адрес по умолчанию, а домен вводится поверх.
            #
            # Таймаут обязателен по той же причине, что в главном меню: сеть
            # может не отвечать, а меню при этом обязано открыться.
            local endpoint def
            def=$(curl -s4 --connect-timeout 2 --max-time 4 https://ipinfo.io/ip 2>/dev/null | tr -d '[:space:]')
            if [[ ! "$def" =~ ^[0-9]{1,3}(\.[0-9]{1,3}){3}$ ]]; then def=""; fi
            if [ -n "$def" ]; then
                echo -e "${C_DESC}Адрес сервера: Enter — взять ${def}, либо введите домен.${NC}"
            else
                echo -e "${C_WARN}Внешний адрес определить не удалось — введите его вручную.${NC}"
            fi
            read -p "Адрес сервера для клиента${def:+ [$def]}: " endpoint
            endpoint="${endpoint:-$def}"
            # Значение попадает в конфиг, который человек копирует к себе, —
            # чистим от управляющих символов, как и остальные адреса.
            endpoint=$(_addr_clean "$endpoint")
            if [ -z "$endpoint" ]; then
                echo -e "${C_DANGER}❌ Без адреса конфиг работать не будет.${NC}"; read -p "Enter..."; return
            fi
