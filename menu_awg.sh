...
function awg_clients {
    ...
    case "$ch" in
        1)
            ...
            #
            # 0.0.0.0/0 — то, чего ждут от VPN. IPv6 сюда НЕ добавляем: у
            # туннеля адреса IPv6 нет, и `::/0` не завернул бы трафик, а
            # похоронил его. Обратная сторона честная: IPv6 у клиента пойдёт
            # мимо туннеля, и на серверах с IPv6 это утечка — записано в
            # передаче отдельным хвостом.
            ...
            docker exec -e AWG_ENDPOINT="${endpoint}:${AWG_PORT}" \
                        ...
                        -e AWG_CLIENT_ALLOWED="0.0.0.0/0" \
