...
# ----------------------------------------------------------------------
# Censorcheck запускается напрямую с сайта автора одной командой.
#
# Раньше здесь была своя копия скрипта с самодельным флагом --mode и тремя
# отдельными пунктами меню. Разбиение существовало только потому, что из
# копии был убран RIPE Atlas ключ автора, и радар ТСПУ без своего ключа
# молча пропускался — выглядело это как неработающие пункты.
#
# ВНИМАНИЕ: у скрипта автора ключ RIPE зашит внутри, и рядом стоит его
# просьба не использовать этот ключ в сторонних проектах. Запуская отсюда,
# мы расходуем именно его квоту. Решение принято сознательно ради того,
# чтобы проверка работала целиком и без настройки.
...
function run_censorcheck {
    ...
    local tmp; tmp=$(mktemp /tmp/censorcheck.XXXXXX.sh)
...
function run_tests_menu {
    while true; do
        ...
        echo -e "${YELLOW}2) 🧪  Censorcheck (геоблок | радар ТСПУ)${NC}"
